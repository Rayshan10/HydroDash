...
<head>
    ...
    <title>Laporan Harian - HydroDash</title>
...
<body class="bg-gray-100 p-8">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                ...
                <h1 class="text-3xl font-bold text-green-700">Laporan Harian</h1>
                <p class="text-gray-600">{{ $date->format('d MMMM Y') }}</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('report.pdf-daily', ['date' => $date->format('Y-m-d')]) }}"
                    class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg">
                    📄 Export PDF
                </a>
                <a href="{{ route('report.export-daily', ['date' => $date->format('Y-m-d')]) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white p-4 rounded-lg shadow-md">
                <p class="text-gray-600 font-semibold mb-2">Pompa pH (Aktif)</p>
                <p class="text-3xl font-bold text-green-600">{{ $stats['pompa_ph_on'] }} <span class="text-sm text-gray-500">/ {{ $stats['total_records'] }}</span></p>
            </div>
            <div class="bg-white p-4 rounded-lg shadow-md">
                <p class="text-gray-600 font-semibold mb-2">Pompa TDS (Aktif)</p>
                <p class="text-3xl font-bold text-green-600">{{ $stats['pompa_tds_on'] }} <span class="text-sm text-gray-500">/ {{ $stats['total_records'] }}</span></p>
            </div>
            <div class="bg-white p-4 rounded-lg shadow-md">
                <p class="text-gray-600 font-semibold mb-2">Pendingin (Aktif)</p>
                <p class="text-3xl font-bold text-blue-600">{{ $stats['pendingin_on'] }} <span class="text-sm text-gray-500">/ {{ $stats['total_records'] }}</span></p>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-8">
            <h3 class="font-bold text-gray-700 mb-6 text-center text-xl">Grafik Tren Sensor</h3>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-4 border-b bg-gray-50">
                <h3 class="font-bold text-gray-700">Riwayat Data Sensor ({{ $data->count() }} records)</h3>
            </div>
                    ...
                    <tbody>
                        @forelse($data as $item)
                            <tr class="border-b hover:bg-gray-50 transition">
                                <td class="p-4 font-medium">{{ $item->created_at->timezone('Asia/Jakarta')->format('H:i:s') }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="p-8 text-center text-gray-400 italic">Tidak ada data untuk tanggal ini</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const labels = {!! json_encode($logs->pluck('created_at')->map(fn($d) => $d->timezone('Asia/Jakarta')->format('H:i:s'))->values()) !!};
